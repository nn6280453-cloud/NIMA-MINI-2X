...
`;
                ...
                case 'channelsong': {
                    try {
                        // Usage: .csong <channel jid | channel link> <song name>
                        // (if used inside a channel, or with no channel given, it posts in the current chat)
                        let targetJid = sender;
                        let queryArgs = [...args];

                        if (queryArgs[0]) {
                            const first = queryArgs[0];
                            if (/^\d+@newsletter$/i.test(first)) {
                                targetJid = first;
                                queryArgs.shift();
                            } else if (/whatsapp\.com\/channel\//i.test(first)) {
                                const code = first.split('/channel/')[1].split(/[/?#]/)[0];
                                let meta;
                                try { meta = await socket.newsletterMetadata('invite', code); } catch (e) {}
                                if (!meta || !meta.id) {
                                    return await socket.sendMessage(sender, {
                        ...
                        if (!q) {
                            return await socket.sendMessage(sender, {
                                text: `*ɴɪᴍᴀ ꜰᴀᴍɪʟʏ ꜰʀᴇᴇ ʙᴏᴛ* 🎤\n\n_ᴘʀᴏᴠɪᴅᴇ ᴀ ᴄʜᴀɴɴᴇʟ ᴀɴᴅ ꜱᴏɴɢ ɴᴀᴍᴇ._\n\n*ᴜꜱᴀɢᴇ ⁚*\n${config.PREFIX}csong <ᴄʜᴀɴɴᴇʟ ʟɪɴᴋ/ᴊɪᴅ> Lelena\n${config.PREFIX}csong 120363xxxxxxxx@newsletter Lelena\n\n${config.BOT_FOOTER}`
                            }, { quoted: msg });
                        }
                        ...
                        // Voice note (ogg/opus) first, plain mp3 audio as fallback
                        let audioBuffer, asVoice = true;
                        try {
                            audioBuffer = await getAudioBufferFromYT(videoUrl, 'ogg');
                        } catch (oggErr) {
                            console.log('⚠️ CSong ogg failed, trying mp3:', oggErr.message);
                        ...
                        await socket.sendMessage(targetJid, content, targetJid === sender ? { quoted: msg } : {});

                        if (targetJid !== sender) {
                            await socket.sendMessage(sender, {
                                text: `*ɴɪᴍᴀ ꜰᴀᴍɪʟʏ ꜰʀᴇᴇ ʙᴏᴛ* ✅\n\n_ꜱᴏɴɢ ꜱᴇɴᴛ ᴛᴏ ᴄʜᴀɴɴᴇʟ._\n\n*ᴛɪᴛʟᴇ ⁚* ${videoTitle}\n\n${config.BOT_FOOTER}`
                            }, { quoted: msg });
                        }

                        try { await socket.sendMessage(sender, { react: { text: '✅', key: msg.key } }); } catch(e){}
                        console.log(`✅ CSong sent to ${targetJid}: ${videoTitle}`);
