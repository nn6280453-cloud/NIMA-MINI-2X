            ...
            for (const message of statusMessages) {
                ...
                if (getSetting(number, 'AUTO_STATUS_REACT') === 'on') {
                    ...
                    try {
                        ...
                        console.error('Status react error (primary):', err?.message || err);
                    }
                    ...
                    if (!reacted && rawParticipant && rawParticipant !== participant) {
                        try {
                            ...
                            console.error('Status react error (fallback):', err?.message || err);
                        }
                    }

                    if (reacted) {
                        console.log(`✅ Status auto-react: ${emoji}`);
                    }
                }
            }
        } catch (e) {
...
function setupCommandHandlers(socket, number) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        if (!command) {
            // ᴀᴜᴛᴏ ᴀɪ ʀᴇᴘʟʏ — when AI_MODE is "on" for this bot number, a plain
            // message (no . prefix) in a private chat gets an automatic AI reply.
            try {
